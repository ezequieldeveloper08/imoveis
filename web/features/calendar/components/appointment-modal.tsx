...
const appointmentSchema = z.object({
  ...
  leadId: z.string().min(1, 'Selecione o visitante'),
...
export function AppointmentModal({ isOpen, onClose, propertyId, leads, onSuccess }: AppointmentModalProps) {
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      // Combine date and time for the backend if needed, 
      // or just send separately if the backend supports it.
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        >
          ...
          <div className="px-8 py-6 border-b border-grey-15 flex items-center justify-between bg-grey-08/50">
            <div>
              <h2 className="text-xl font-bold text-white">Agendar Visita</h2>
              <p className="text-xs text-grey-40">Marque uma visita para apresentar o imóvel.</p>
            </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="p-8 space-y-6">
              ...
              <Select onValueChange={(val) => setValue('leadId', val)}>
                <SelectTrigger className="h-14 bg-grey-08 border-grey-15 text-white">
                  <div className="flex items-center gap-3">
                    ...
                    <SelectValue placeholder="Quem irá visitar?" />
                  </div>
                </SelectTrigger>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                {errors.time && <p className="text-red-400 text-xs mt-1">{errors.time.message as string}</p>}
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                <Info className="absolute left-4 top-4 h-4 w-4 text-grey-40 z-10" />
                <textarea 
                  id="description" 
                  {...register('description')}
                  placeholder="Ex: Cliente virá com a esposa, apresentar área de lazer..."
            ...
            <div className="flex gap-4 pt-2">
              ...
              >
                {isSubmitting ? <Loader2 className="h-5 w-5 animate-spin" /> : 'Agendar Visita'}
              </Button>
            </div>
          </form>
        </motion.div>
