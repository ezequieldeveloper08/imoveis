  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {isLoading ? (
          <div className="col-span-full h-64 flex flex-col items-center justify-center bg-grey-10 border border-grey-15 rounded-2xl border-dashed">
            <div className="h-10 w-10 border-2 border-purple-60 border-t-transparent rounded-full animate-spin mb-4" />
            <p className="text-grey-60 font-bold uppercase text-[10px] tracking-widest">Carregando estrutura...</p>
          </div>
        ) : departments.length === 0 ? (
          <div className="col-span-full h-64 flex flex-col items-center justify-center bg-grey-10 border border-grey-15 rounded-2xl border-dashed opacity-50">
            <Network className="h-12 w-12 text-grey-40 mb-4" />
            <p className="text-grey-60 font-bold uppercase text-[10px] tracking-widest">Nenhum departamento encontrado</p>
          </div>
        ) : (
            ...
            <div key={dept.id} className="bg-grey-10 border border-grey-15 rounded-2xl p-6 hover:border-grey-30 transition-all group relative overflow-hidden">
              ...
              <div className="flex items-start justify-between mb-6">
                ...
                <div className="flex gap-2">
                  <button 
                    ...
                    className="h-8 w-8 flex items-center justify-center text-grey-60 hover:text-red-400 transition-colors"
                  >
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between p-3 bg-grey-08 rounded-xl border border-grey-15">
                  ...
                  <span className="text-xs text-white font-bold">{dept.manager || 'Não definido'}</span>
                </div>
                ...
                <div className="flex items-center justify-between p-3 bg-grey-08 rounded-xl border border-grey-15">
                  ...
                  <span className="text-xs text-white font-bold">{dept.memberCount || 0} colaboradores</span>
                </div>
              </div>
